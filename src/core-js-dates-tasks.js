...
function getNextFriday(date) {
  ...
  const daysToAdd = (5 - resultDate.getDay() + 7) % 7;
  const daysUntilNextFriday = daysToAdd === 0 ? 7 : daysToAdd;
...
function formatDate(date) {
  ...
  return `${month}/${day}/${year}, ${hour}:${paddedMinute}:${paddedSecond} ${amPm}`;
}
...
function getNextFridayThe13th(date) {
  ...
  for (let month = startMonth; month < 12; month += 1) {
    const checkDate = new Date(year, month, 13);
    if (checkDate.getDay() === 5) {
      return checkDate;
    }
  }

  for (let month = 0; month <= startMonth; month += 1) {
    const checkDate = new Date(year + 1, month, 13);
    if (checkDate.getDay() === 5) {
      return checkDate;
    }
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDay <= endDate) {
    if (cycleDay < countWorkDays) {
      workSchedule.push(scheduleFormatDate(currentDay));
    }
    cycleDay += 1;
    if (cycleDay === countWorkDays + countOffDays) cycleDay = 0;
